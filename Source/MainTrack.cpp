...
#pragma region Imag
    juce::File windowImg("C:/Ryuichi/UI_Image/TrackBar.png"); //절대경로 불러오기
    if (windowImg.existsAsFile()) //정상인지 조회
    {
        juce::Image img = juce::ImageFileFormat::loadFrom(windowImg); //image 저장
        WindowBarComponent.setImage(img);//이미지 변수에 저장
        addAndMakeVisible(&WindowBarComponent); //이미지 띠우기
    }
